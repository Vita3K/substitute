...
#endif
#else
/* Vita3K: We only use transform-dis, not the dynamic linker APIs.
   Define a dummy type so the header compiles on non-Apple platforms. */
struct substitute_sym_dummy { int n_value; };
typedef struct substitute_sym_dummy substitute_sym;
#endif
...
#else
    int dummy; /* MSVC requires at least one member in C */
#endif
